  ...
  function load() {
    ...
    var globalPaths = ""; // module-deps doesn't search server npm (perhaps client npm)
  	...
  	sysModules.forEach(function(module) {
  		...
  		console.log("CACHING ---",module.path,logicalPath);
  		cache[module.path] = { 
  			id: module.name,
  			deps: {},
  			file: module.file,
    ...
    sysModules.forEach(function(module) {
  		var logicalPath = path.relative(clientRoot,module.path);
      try {
      bundler.browserify.require(logicalPath, {expose:module.name});
      // bundler.browserify.external(module.name);
      }
      catch(err) {
    ...
    client.paths.code.forEach(function(rel) {
	    ...
	    bundler.browserify.require(rel,{expose:rel});
    });
    // bundler.browserify.external('socketstream');
    // bundler.browserify.external('eventemitter2');

    this.browserify.on('file',function(file,id,parent){
    	var f = file;
    });
    this.browserify.pipeline.on('file',function(file,id,parent){
    	var f = file;
    });
